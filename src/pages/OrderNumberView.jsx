...
function OrderNumberView({ userRole }) {
  ...
  const showAllocatedNumbers = ['amount paid', 'delivered'].includes(orderData?.orderStatus?.toLowerCase());

  // Check if allocated numbers are editable
  const canEditAllocatedNumbers = userRole === 'Internal' && orderData?.orderStatus?.toLowerCase() === 'amount paid';
  ...
  const handleAddAllocatedNumber = async () => {
    if (!canEditAllocatedNumbers) {
      toast({
        title: 'Access Denied',
        description: 'Only internal users can add numbers when order status is "Amount Paid"',
        status: 'error',
        duration: 3000,
        isClosable: true,
      });
  ...
  const handleRemoveAllocatedNumber = async (id) => {
    if (!canEditAllocatedNumbers) {
      toast({
        title: 'Access Denied',
        description: 'Only internal users can remove numbers when order status is "Amount Paid"',
        status: 'error',
        duration: 3000,
        isClosable: true,
      });
  ...
  const transformPricingData = (rawPricing) => {
    ...
    return {
      ...
      ppmFix: rawPricing.ppm_fix ? `$${parseFloat(rawPricing.ppm_fix).toFixed(4)}` : '',
      ppmMobile: rawPricing.ppm_mobile ? `$${parseFloat(rawPricing.ppm_mobile).toFixed(4)}` : '',
      ppmPayphone: rawPricing.ppm_payphone ? `$${parseFloat(rawPricing.ppm_payphone).toFixed(4)}` : '',
      ...
      Incomingppm: rawPricing.incoming_ppm ? `$${parseFloat(rawPricing.incoming_ppm).toFixed(4)}` : '',
      Outgoingppmfix: rawPricing.outgoing_ppm_fix ? `$${parseFloat(rawPricing.outgoing_ppm_fix).toFixed(4)}` : '',
      Outgoingppmmobile: rawPricing.outgoing_ppm_mobile ? `$${parseFloat(rawPricing.outgoing_ppm_mobile).toFixed(4)}` : '',
      incmongsms: rawPricing.incoming_sms ? `$${parseFloat(rawPricing.incoming_sms).toFixed(4)}` : '',
      outgoingsms: rawPricing.outgoing_sms ? `$${parseFloat(rawPricing.outgoing_sms).toFixed(4)}` : '',
              ...
              <Grid templateColumns="repeat(5, 1fr)" gap={4} alignItems="center">
                <GridItem>
                  <VStack spacing={1} align="start">
                    ...
                    <Badge bg={"blue.100"}>
                    <Text fontSize="sm" fontWeight="semibold">{orderData.productType}</Text>
                    </Badge>
                  </VStack>
                </GridItem>
              ...
              <Grid templateColumns="repeat(7, 1fr)" gap={4} alignItems="center">
                <GridItem>
                  <VStack spacing={1} align="start">
                    ...
                    <Text fontSize="sm" color="gray.800" fontWeight="bold">{orderData.serviceName}</Text>
                    </Badge>
                  </VStack>
                </GridItem>
                ...
                <GridItem>
                  <VStack spacing={1} align="start">
                    ...
                    <Text fontSize="md" color="gray.800" fontWeight="semibold">{orderData.orderDate.split('T')[0]}</Text>
                  </VStack>
                </GridItem>
                ...
                <GridItem>
                  <VStack spacing={1} align="start">
                    ...
                    <Text fontSize="md" color="gray.800" fontWeight="semibold">{orderData.createdBy}</Text>
                  </VStack>
                </GridItem>
              </Grid>
            )}
          ...
          <Card bg="white" borderRadius="12px" boxShadow="sm" border="1px solid" borderColor="gray.200">
            <CardBody p={6}>
              <HStack justify="space-between" align="start" mb={4}>
                ...
                <HStack px={3} border={"1px solid gray"} borderRadius={"full"} bg={"blue.50"} spacing={8}>
                  <HStack spacing={2} align="center">
                    <Text fontSize="sm" color="gray.600" fontWeight="medium">Allocated</Text>
                    <Text fontSize="2xl" color="blue.600" fontWeight="bold">{allocatedNumbers.length}</Text>
                  </HStack>
                  <HStack spacing={2} align="center">
                    <Text fontSize="sm" color="gray.600" fontWeight="medium">Needed</Text>
                    <Text fontSize="2xl" color="green.600" fontWeight="bold">{orderData.quantity}</Text>
                  </HStack>
                  {allocatedNumbers.length < orderData.quantity && (
                    <HStack spacing={2} align="center">
                      <Text fontSize="sm" color="gray.600" fontWeight="medium">Pending</Text>
                      <Text fontSize="2xl" color="orange.600" fontWeight="bold">{orderData.quantity - allocatedNumbers.length}</Text>
                    </HStack>
                  )}
                </HStack>
              </HStack>
                  ...
                  <Table variant="simple">
                    <Thead bg="gray.300">
                      <Tr>
                        ...
                        {canEditAllocatedNumbers && <Th color="gray.800">Actions</Th>}
                      </Tr>
                    </Thead>
                    <Tbody>
                      {allocatedNumbers.map((number, index) => (
                        <Tr key={number.id}>
                          ...
                          <Td fontWeight="semibold" color="blue.600">{number.number}</Td>
                          <Td>
                            <Badge 
                              colorScheme={number.status === 'Active' ? 'green' : 'gray'}
                              borderRadius="full"
                              px={2}
                              py={1}
                            >
                          ...
                          <Td>{new Date(number.allocatedAt).toLocaleDateString()}</Td>
                          {canEditAllocatedNumbers && (
                            <Td>
                              <IconButton
                                ...
                                colorScheme="red"
                                size="sm"
                                onClick={() => handleRemoveAllocatedNumber(number.id)}
                              />
                            </Td>
                          )}
                        </Tr>
